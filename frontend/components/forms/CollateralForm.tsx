...
export default function CollateralForm() {
  ...
  const {
    ...
    defaultValues: {
      ...
      estimatedValue: wizardData.collateralValue,
  ...
  const onSubmit = (data: CollateralInput) => {
    // Check if at least one ownership document is uploaded
    const documentKeys = Object.keys(files);
    const hasDoc = documentKeys.some((k) => !!files[k]);
